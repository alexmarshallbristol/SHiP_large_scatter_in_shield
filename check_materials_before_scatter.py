...
run.CreateGeometryFile("check_materials_geom.root")
...
muon_scattering_history_pre_scatter = muon_scattering_history_pre_scatter[:,1,:]

# print(muon_scattering_history_pre_scatter)

print(np.shape(muon_scattering_history_pre_scatter))

materials = np.empty(np.shape(muon_scattering_history_pre_scatter)[0])

for i in range(0, np.shape(muon_scattering_history_pre_scatter)[0]):
  # print(muon_scattering_history_pre_scatter[i][1],muon_scattering_history_pre_scatter[i][2],muon_scattering_history_pre_scatter[i][3])
  material = checkMagFields.check_material(muon_scattering_history_pre_scatter[i][1],muon_scattering_history_pre_scatter[i][2],muon_scattering_history_pre_scatter[i][3])
  # print(material)
...
print(muon_scattering_history_pre_scatter[0][3])

print(np.shape(materials))
print(np.shape(np.where(materials == 1)))
np.save('materials_iron_bool',materials)

print('start')
data = np.empty((0,3))

for i in range(-400,400,5):
  for j in range(-400,400,5):
    material = checkMagFields.check_material(i,j,muon_scattering_history_pre_scatter[0][3])
    # print(material)
    if material == 'iron':
      material_in = 1
    else:
      material_in = 0
    data = np.append(data, [[i,j,material_in]], axis=0)

plt.scatter(data[:,0],data[:,1],c=data[:,2],norm=LogNorm(),edgecolors='none')
plt.savefig('shield_shape_at_plane.png')
